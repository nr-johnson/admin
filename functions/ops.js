    ...
    return (req, res, next) => {
        ...
        req.findItem = (db, col, data, index) => {
            return new Promise(async resolve => {
                if(index) {
                    const result = await req.mongo.db(db).collection(col).find(data).collation(index).limit(1).toArray()
                    resolve(result[0])
                } else {
                    const result = await req.mongo.db(db).collection(col).find(data).limit(1).toArray()
                    resolve(result[0])
                }
            })
        }
